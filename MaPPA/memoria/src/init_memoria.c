...
bool cargar_configuraciones( t_config_memoria* config_memoria, t_log* logger) {
    ...
    config_memoria->puerto_fs = config_get_int_value(config_m, "PUERTO_FILESYSTEM");
    ...
    copiar_valor(&config_memoria->ip_fs, config_get_string_value(config_m, "IP_FILESYSTEM"));
...
void iniciar_modulo(t_log* loggerMemoria, t_config_memoria* config_memoria) {
    ...
    int md_memoria = iniciar_servidor(loggerMemoria, "MEMORIA", ip_escucha, puerto_escucha);
